...
def test(rate, protocol, parallel, delay_ms, drop_rate, limit_rate_bytes_per_second, blockcount):
    ...
    args = f' -i 2 -Z -c {IPERF3_SERVER} -p 5103 --json -u --udp-counters-64bit' if protocol == 'UDP' else '-i 2 -c {IPERF3_SERVER} --json'
...
def test_suite():
    ...
    json_path = Path(f'./results/{utc_now:%Y.%m.%d.%H.%M.%S}_5103_results.json')
